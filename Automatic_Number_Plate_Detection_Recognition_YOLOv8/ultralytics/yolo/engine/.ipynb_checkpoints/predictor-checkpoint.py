...
class BasePredictor:
    ...
    # def write_results(self, i, preds, batch):
    #     p, im, im0 = batch  # path, image, original image
    #     annotator = self.get_annotator(im0)
        
    #     plates_detected = False  # Flag para verificar se uma placa foi detectada neste frame
    
    #     # Iterate through detections and draw boxes
    #     for det in preds:
    #         if len(det):
    #             for *xyxy, conf, cls in reversed(det):
    #                 # Verifica se a classe é correspondente a uma placa
    #                 if self.model.names[int(cls)] == 'plate':
    #                     plates_detected = True
    #                     label = f'{self.model.names[int(cls)]} {conf:.2f}'
    #                     annotator.box_label(xyxy, label, color=(255, 0, 0))  # Red color for plates
    
    #     if plates_detected:
    #         # Save annotated image in a specific directory
    #         annotated_dir = self.save_dir / 'frames_annotated'
    #         annotated_dir.mkdir(parents=True, exist_ok=True)
    #         save_path = annotated_dir / f"{p.stem}_annotated.jpg"
    #         cv2.imwrite(str(save_path), annotator.result())
    #         LOGGER.info(f"Saved annotated frame to {save_path}")
    
    #     return plates_detected  # Retorna True se uma placa foi detectada neste frame, senão retorna False
