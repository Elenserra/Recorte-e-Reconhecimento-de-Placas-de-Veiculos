...
from ultralytics.yolo.utils import check_imgsz
...
def check_imgsz(imgsz, min_dim):
    """
    Check if the input image size meets the minimum dimension requirement.

    Args:
        imgsz (int or tuple): Size of the input image. If a tuple, it should be in (width, height) format.
        min_dim (int): Minimum dimension required for the image.

    Returns:
        tuple: Resized image size to meet the minimum dimension requirement.
    """
    if isinstance(imgsz, int):
        imgsz = (imgsz, imgsz)
    elif isinstance(imgsz, tuple):
        if len(imgsz) != 2:
            raise ValueError("imgsz must be an integer or a tuple of length 2 (width, height).")
    else:
        raise ValueError("imgsz must be an integer or a tuple of length 2 (width, height).")

    width, height = imgsz
    if min(width, height) < min_dim:
        ratio = min_dim / min(width, height)
