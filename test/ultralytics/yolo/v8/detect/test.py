...
class DetectionValidator(BaseValidator):
    ...
    def init_metrics(self, model):
        ...
        test = self.data.get('test', '')  # test path
        self.is_coco = isinstance(test, str) and test.endswith(f'coco{os.sep}test2017.txt')  # is COCO dataset
    ...
    def get_dataloader(self, dataset_path, batch_size):
        ...
        return create_dataloader(path=dataset_path,
            ...
            build_dataloader(self.args, batch_size, img_path=dataset_path, stride=gs, mode="test")[0]
    ...
    def plot_val_samples(self, batch, ni):
        plot_images(batch["img"],
                    ...
                    batch["bboxes"],
                    paths=batch["im_file"],
                    fname=self.save_dir / f"test_batch{ni}_labels.jpg",
                    names=self.names)
    ...
    def plot_predictions(self, batch, preds, ni):
        plot_images(batch["img"],
                    *output_to_target(preds, max_det=15),
                    paths=batch["im_file"],
                    fname=self.save_dir / f'test_batch{ni}_pred.jpg',
    ...
    def eval_json(self, stats):
        if self.args.save_json and self.is_coco and len(self.jdict):
            anno_json = self.data['path'] / "annotations/instances_test2017.json"  # annotations
